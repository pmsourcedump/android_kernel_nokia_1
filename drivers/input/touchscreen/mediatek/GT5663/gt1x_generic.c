...
u8 gt1x_config[GTP_CONFIG_ORG_LENGTH + GTP_CONFIG_EXT_LENGTH] = { 0 };
...
static ssize_t gt1x_fw_ver_read_proc(struct file *file, char __user * page, size_t size, loff_t * ppos);
static ssize_t gt1x_fw_ver_write_proc(struct file *file, const char *buffer, size_t count, loff_t * ppos);
static struct proc_dir_entry *gt1x_fw_ver_proc_entry = NULL;
static const struct file_operations gt1x_fw_ver_fops = {
	.owner = THIS_MODULE,
	.read = gt1x_fw_ver_read_proc,
...
static s32 gt1x_init_fw_ver_node(void)
{
	...
	GTP_INFO("Created proc entry /proc/%s.", GT1X_FW_VER_PROC_FILE);
	return 0;
}
...
static ssize_t gt1x_fw_ver_read_proc(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	...
	if(ret != 0){
		GTP_ERROR("Copy to user error!copy sucess length = %d < %d", ret, length);
		length = ret;
	}
...
static ssize_t gt1x_fw_ver_write_proc(struct file *file, const char *buffer, size_t count, loff_t * ppos)
{
	GTP_INFO("Write fireware version ok");
	return 0;
}

static void gt1x_deinit_fw_ver_node(void)
{
	if (gt1x_fw_ver_proc_entry != NULL) {
		remove_proc_entry(GT1X_FW_VER_PROC_FILE, NULL);
	}
}
...
static ssize_t gt1x_debug_read_proc(struct file *file, char __user * page, size_t size, loff_t * ppos)
{
	...
	i = gt1x_i2c_read(GTP_REG_CONFIG_DATA, temp_data, gt1x_cfg_length);
...
static ssize_t gt1x_debug_write_proc(struct file *file, const char *buffer, size_t count, loff_t * ppos)
{
	...
	if (count > gt1x_cfg_length) {
		GTP_ERROR("Too much data, buffer size: %d, data:%ld", gt1x_cfg_length, (unsigned long)count);
		return -EFAULT;
	}
	...
	sscanf(buf, "%49s %d", (char *)&mode_str, &mode);
...
#endif
	sscanf(buf, "%49s %s", (char *)&mode_str, (char *)&arg1);
...
s32 gt1x_init_panel(void)
{
	...
	if (sensor_id >= 6 || cfg_lens[sensor_id] < GTP_CONFIG_ORG_LENGTH) {
		sensor_id = 0;
	...
	if (cfg_len > GTP_CONFIG_ORG_LENGTH
			&& cfgs[sensor_id][0x805A - GTP_REG_CONFIG_DATA] & 0x40) {
		/* extends config */
	...
	if (gt1x_config[0x805A - GTP_REG_CONFIG_DATA] & 0x40) {
...
{
	s32 ret = -1;
#ifndef CONFIG_GTP_DRIVER_SEND_CFG
	s32 ret_tmp = 0;
#endif
	u8 rqst_data = 0;
...
#else
			ret = 0;
			ret_tmp = gt1x_i2c_write(GTP_REG_CONFIG_DATA, gt1x_config, GTP_CONFIG_ORG_LENGTH);
			if (ret_tmp < 0){
				ret = -1;
			}
			/* extends config */
			if (gt1x_config[0x805A - GTP_REG_CONFIG_DATA] & 0x40) {
				ret_tmp = gt1x_i2c_write(GTP_REG_EXT_CONFIG,
						&gt1x_config[GTP_CONFIG_ORG_LENGTH], GTP_CONFIG_EXT_LENGTH);
...
s32 gt1x_touch_event_handler(u8 * data, struct input_dev * dev, struct input_dev * pen_dev)
{
	...
	//GTP_DEBUG("tpd_dts_data.(%d)", tpd_dts_data.use_tpd_button);
...
static s32 gt1x_ps_operate(void *self, u32 command, void *buff_in, s32 size_in, void *buff_out, s32 size_out, s32 * actualout)
{
	...
	s32 value;
	struct hwm_sensor_data *sensor_data;
	struct hwm_sensor_data sensor_size;//calculate the size of struct hwm_sensor_data:sizeof(sensor_size)
	...
	do{
		...
		*len = cfg_lens[sensor_id];
		if (*len == 0 || *len != gt1x_cfg_length) {
			memset(cfg, 0, GTP_CONFIG_ORG_LENGTH + GTP_CONFIG_EXT_LENGTH);
			*len = 0;
 ...
 */
#ifdef CONFIG_GTP_CHARGER_SWITCH
static u8 gt1x_config_charger[GTP_CONFIG_ORG_LENGTH + GTP_CONFIG_EXT_LENGTH] = { 0 };
#ifdef CONFIG_GTP_USE_PMIC_DETECT_USB_PLUGIN
bool   gtp_usb_plugin_enable = false;
#else
...
#ifdef CONFIG_GTP_USE_PMIC_DETECT_USB_PLUGIN
void gt1x_charger_work_func(void);
#else
...
{
#ifndef CONFIG_GTP_USE_PMIC_DETECT_USB_PLUGIN
...
#ifdef CONFIG_GTP_USE_PMIC_DETECT_USB_PLUGIN
	GTP_INFO("firefly charger usb is enable = %d (0 disable 1 enable)\n", gtp_usb_plugin_enable);
	if (gtp_usb_plugin_enable) {
#else
