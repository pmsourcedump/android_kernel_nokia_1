...
static int tpd_irq_flag = 1;//when request irq, irq was enable
...
extern void mt65xx_eint_registration(kal_uint8 eintno, kal_bool Dbounce_En, kal_bool ACT_Polarity,
		void (EINT_FUNC_PTR) (void), kal_bool auto_umask);
#endif
...
static struct i2c_board_info __initdata i2c_tpd = {
	I2C_BOARD_INFO(GTP_DRIVER_NAME,
			(GTP_I2C_ADDRESS >> 1))
};
#endif
...
void gt1x_irq_disable(void)
{
	...
	spin_lock_irqsave(&irq_lock, flag);
	if (tpd_irq_flag) {
		GTP_DEBUG("gt1x_irq_disable is %d", tpd_irq_flag);
#ifdef GTP_CONFIG_OF
...
return 0;
	...
	//gt1x_irq_enable();  //tpd_irq_registration() already enable the irq,so delete it for unblanced enable.
...
#if (defined CONFIG_GTP_PROXIMITY) || (defined CONFIG_HOTKNOT_BLOCK_RW)
exit_work_func:
#endif
...
static int tpd_local_init(void)
{
	...
	switch(boot_mode){
		...
		case LOW_POWER_OFF_CHARGING_BOOT:
			GTP_ERROR("Chargeing  mode, Disable TP driver");
			return -1;
#endif
		case RECOVERY_BOOT:
			GTP_ERROR("Recovery mode, Disable TP driver");
			return -1;
		default:
			break;
	}
