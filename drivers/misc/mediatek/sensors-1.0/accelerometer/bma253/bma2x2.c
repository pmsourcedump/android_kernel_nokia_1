...
#define GSE_DEBUG(fmt, args...)   pr_debug(GSE_TAG fmt, ##args)


#define COMPATIABLE_NAME "mediatek,bma255"

static int cali_offset[3] = {0,0,0};
#define GRAVITY  (9806)
#define XYZ_MAX_BIAS 1470 //(GRAVITY * 0.15)
...
/*----------------------------------------------------------------------------*/
static int BMA2x2_ReadSensorData_for_cali(struct i2c_client *client,
	 char *buf)
{
	struct bma2x2_i2c_data *obj =
	 (struct bma2x2_i2c_data *)i2c_get_clientdata(client);
	...
	if (res) {
		...
		acc[BMA2x2_AXIS_Z] =
		 ...
		 / obj->reso->sensitivity;
#if 0
		GSE_DEBUG("Mapped gsensor data: %d, %d, %d!\n",
		 acc[BMA2x2_AXIS_X],
		 acc[BMA2x2_AXIS_Y],
		 acc[BMA2x2_AXIS_Z]);
#endif
	 ...
	 char *buf, int bufsize)
{
	...
	if (res) {
		...
		acc[BMA2x2_AXIS_X] -= cali_offset[BMA2x2_AXIS_X];
		acc[BMA2x2_AXIS_Y] -= cali_offset[BMA2x2_AXIS_Y];
		acc[BMA2x2_AXIS_Z] -= cali_offset[BMA2x2_AXIS_Z];
...
static int BMA2x2_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	if (res) {
		...
		sprintf(buf, "%04x %04x %04x", databuf[BMA2x2_AXIS_X],
...
static ssize_t show_cali_value(struct device_driver *ddri, char *buf)
{
	...
	len = snprintf(buf, PAGE_SIZE, "%d %d %d\n",
		 cali_offset[BMA2x2_AXIS_X],
		 cali_offset[BMA2x2_AXIS_Y],
		 cali_offset[BMA2x2_AXIS_Z]);
...
{
	int x, y, z;
	int sum_data[BMA2x2_AXES_NUM] = {0,0,0}, avg_data[BMA2x2_AXES_NUM];
	char buff[BMA2x2_BUFSIZE];
	int loop_times = 8;
	int max_loop_times = loop_times * 3;
#if 0
	...
	while(loop_times) {
		max_loop_times--;
		if (0 == max_loop_times){
			GSE_LOG("Have loop 30 times, Calibrate fail\n");
			return -1;
		}
		BMA2x2_ReadSensorData_for_cali(obj_i2c_data->client, buff);
		sscanf(buff, "%11x %11x %11x", &x, &y, &z);
	#if 0
		if((x > (0 + XYZ_MAX_BIAS)) || (x < (0 - XYZ_MAX_BIAS)) ||
			(y > (0 + XYZ_MAX_BIAS)) || (y < (0 - XYZ_MAX_BIAS)) ||
			(x > (GRAVITY + XYZ_MAX_BIAS)) || (x < (GRAVITY - XYZ_MAX_BIAS))
			){
			//if the deviation greater then 15%, discard this one.
			continue;
		}
	#endif
	    GSE_ERR("[FIH]%s loop_times(%d) (%d %d %d)\n", __func__, loop_times, x, y, z);
		sum_data[0] += x;
		sum_data[1] += y;
		sum_data[2] += z;
		loop_times--;
	...
	avg_data[0] = sum_data[0] >> 3;
	avg_data[1] = sum_data[1] >> 3;
	avg_data[2] = sum_data[2] >> 3;

	GSE_LOG("[FIH]%s avg(%d %d %d)\n", __func__,
			avg_data[0],
			avg_data[1],
			avg_data[2]);

	cali_offset[0] = avg_data[0] - 0;
	cali_offset[1] = avg_data[1] - 0;
	cali_offset[2] = avg_data[2] - GRAVITY;
	GSE_LOG("[FIH]%s offset(%d %d %d)\n", __func__,
			cali_offset[0],
			cali_offset[1],
			cali_offset[2]);
	 ...
	 unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_SET_CALI:
		...
		if (copy_from_user(&cali_data, data, sizeof(cali_data))) {
			err = -EFAULT;
			break;
		}
		if (atomic_read(&obj->suspend)) {
			...
			cali_offset[BMA2x2_AXIS_X] = cali_data[0]/1000;
			cali_offset[BMA2x2_AXIS_Y] = cali_data[1]/1000;
			cali_offset[BMA2x2_AXIS_Z] = cali_data[2]/1000;
		}
		break;
...
static int bma2x2_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	GSE_LOG("bma2x2 acc set delay = (%lld) ok.\n", samplingPeriodNs);
	return 0;
}

static int bma2x2_flush(void)
{
	 ...
	 const struct i2c_device_id *id)
{
	...
	ctl.batch = bma2x2_batch;
	ctl.flush = bma2x2_flush;
