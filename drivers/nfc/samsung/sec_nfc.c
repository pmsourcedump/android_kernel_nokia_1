...
#define SEC_NFC_DTS_NAME "samsung,nfc"
...
extern char nfc_hw_support[16]; /* defined in proc_info.c */
...
/*static int sec_nfc_chip_detect(struct sec_nfc_info *info)
{
	...
	memcpy(I2CDMABuf, get_boot_info_cmd, sizeof(get_boot_info_cmd));

	client->addr = client->addr & I2C_MASK_FLAG;
	client->ext_flag |= I2C_DMA_FLAG;
	ret = i2c_master_send(client,
		(unsigned char *)(uintptr_t) I2CDMABuf_pa, sizeof(get_boot_info_cmd));
	...
	client->addr = client->addr & I2C_MASK_FLAG;
	client->ext_flag |= I2C_DMA_FLAG;
	ret = i2c_master_recv(client, (unsigned char *)(uintptr_t) I2CDMABuf_pa, sizeof(rx_buff));
	...
	memcpy(rx_buff, I2CDMABuf, sizeof(rx_buff));
	if (rx_buff[0] != 0x81 ||
		rx_buff[1] != 0x00 ||
		rx_buff[2] != 0x0E ||
		rx_buff[3] != 0x00) {
	...
	struct sec_nfc_platform_data *pdata)
{
...
#endif
       */
	node = of_find_compatible_node(NULL, NULL, "samsung,nfc-sec");
        if (node) {
		of_property_read_u32_array(node, "ven-gpio",
					   &(pdata->ven), 1);
		of_property_read_u32_array(node, "firm-gpio",
					   &(pdata->firm), 1);
                of_property_read_u32_array(node, "pvdd_en-gpio",
                                           &(pdata->tvdd), 1);
#ifdef CONFIG_SEC_NFC_IF_I2C
	...
	pr_info("%s: irq : %d, ven : %d, firm : %d, pvdd_en : %d\n",
			__func__, pdata->irq, pdata->ven, pdata->firm, pdata->tvdd);
	return 0;
}
...
static int __sec_nfc_probe(struct device *dev)
{
	...
	if (!pdata) {
        ...
        ret = gpio_request(pdata->tvdd, "nfc_pvdd_en");
	if (ret) {
		dev_err(dev, "failed to get gpio pvdd_en\n");
		goto err_gpio_pvdd_en;
	}
...
static int __sec_nfc_remove(struct device *dev)
{
	...
	sec_nfc_set_mode(info, SEC_NFC_MODE_OFF);
	/*gpio_direction_input(pdata->ven);*/
	gpio_set_value(pdata->ven, 0);
	gpio_free(pdata->ven);
	/* TODO: Set GPIO0 to 0 make usb unavailable*/
	/*gpio_set_value(pdata->tvdd, 0);*/
	gpio_free(pdata->tvdd);
	if (pdata->firm) {
		/*gpio_direction_input(pdata->firm);
...
/* if nfc is not supported, set nfc gpio input and pull-down */
static bool is_nfc_hw_support(void)
{
	struct device_node *node = NULL;
	unsigned int ven_gpio = 0, firm_gpio = 0, irq_gpio = 0;
	unsigned int clk_req_gpio = 0;

	if(0 == strncmp(nfc_hw_support,"Support",7)) {
		return true;
	}
	else {
		node = of_find_compatible_node(NULL, NULL, "samsung,nfc-sec");
		...
		/*ven_gpio*/
		mt_set_gpio_mode(ven_gpio, GPIO_MODE_GPIO);
		mt_set_gpio_dir(ven_gpio, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(ven_gpio, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(ven_gpio, GPIO_PULL_DOWN);

		/*firm_gpio*/
		mt_set_gpio_mode(firm_gpio, GPIO_MODE_GPIO);
		mt_set_gpio_dir(firm_gpio, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(firm_gpio, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(firm_gpio, GPIO_PULL_DOWN);

		/*irq_gpio*/
		mt_set_gpio_mode(irq_gpio, GPIO_MODE_GPIO);
		mt_set_gpio_dir(irq_gpio, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(irq_gpio, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(irq_gpio, GPIO_PULL_DOWN);

		/*clk_req_gpio*/
		mt_set_gpio_mode(clk_req_gpio, GPIO_MODE_GPIO);
		mt_set_gpio_dir(clk_req_gpio, GPIO_DIR_IN);
		mt_set_gpio_pull_enable(clk_req_gpio, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(clk_req_gpio, GPIO_PULL_DOWN);
