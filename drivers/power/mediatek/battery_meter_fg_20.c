...
/*Jason: BAT_ID************************************
		BAT_ID   voltage      resistance
			0   -- 900mV ---100K
			1  -- 1/3 1.8V 600mV --47k

			...
		Positive and negative range 200mV
**************************************************/
#define	POSITIVE_AND_NEGATIVE_RANGE	150
signed int g_battery_id_voltage[TOTAL_BATTERY_NUMBER+1] = {900,1200,-1};

#if defined(MTK_MULTI_BAT_PROFILE_SUPPORT) || defined(BATTERY_ID_CHECK)
...
void fgauge_get_profile_id(void)
{
	...
	//ret=MM_GetOneChannelValue_Cali(BATTERY_ID_CHANNEL_NUM, &id_volt);
	...
	if (((sizeof(g_battery_id_voltage) / sizeof(signed int))-1) != TOTAL_BATTERY_NUMBER) {
		bm_info("[fgauge_get_profile_id]error! voltage range incorrect!\n");
		return;
	}

	while (time++ < 5) {
		battery_meter_ctrl(BATTERY_METER_CMD_GET_ADC_V_BAT_ID, &id_volt);
		for (id = 0; id < TOTAL_BATTERY_NUMBER+1; id++) {
			if (g_battery_id_voltage[id] == -1) {
				g_fg_battery_id = 0; //use bat_id_0 battery's zcv.
				meter_to_common_battery_id(255);//for FTM
				bm_print(BM_LOG_CRTI, "[%s]: check bat_ID error, use default bat_id_0\n",__func__);
			} else if ((id_volt > (g_battery_id_voltage[id] - POSITIVE_AND_NEGATIVE_RANGE))
				&& (id_volt < (g_battery_id_voltage[id] + POSITIVE_AND_NEGATIVE_RANGE))) {
				g_fg_battery_id = id;
...
int force_get_tbat_pse(kal_bool update)
{
...
{
	return force_get_tbat_pse(update)/10;
}
...
signed int battery_meter_get_battery_temperature_pse(void)
{
#ifdef MTK_BATTERY_LIFETIME_DATA_SUPPORT
	signed int batt_temp = force_get_tbat_pse(KAL_TRUE);

	if (batt_temp/10 > gFG_max_temperature)
		gFG_max_temperature = batt_temp/10;
	if (batt_temp/10 < gFG_min_temperature)
		gFG_min_temperature = batt_temp/10;

	return batt_temp;
#else
	return force_get_tbat_pse(KAL_TRUE);
#endif
}
